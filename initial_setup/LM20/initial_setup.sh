...
sudo sed -i -e "/http:\/\/packages.linuxmint.com\/s/^/#/" /etc/apt/sources.list.d/official-package-repositories.list > /etc/apt/sources.list.d/official-package-repositories.list
sudo sed -i -e "/http:\/\/archive.ubuntu.com/ubuntu\/s/^/#/g" /etc/apt/sources.list.d/official-package-repositories.list > /etc/apt/sources.list.d/official-package-repositories.list
...
sudo su
sed -i -e '/GRUB_CMDLINE_LINUX_DEFAULT=/s/".*"/"quiet splash acpi_backlight=video acpi_osi=!!"/g' /etc/default/grub > /etc/default/grub
...
sed -i -e "/\[Element Speaker\]/{N;N;N;s/volume = merge/volume = off/g}" /usr/share/pulseaudio/alsa-mixer/paths/analog-output-speaker.conf > /usr/share/pulseaudio/alsa-mixer/paths/analog-output-speaker.conf
...
sed -i -e "/^#\(Fallback\)\?NTP=/s/^#//" /etc/systemd/timesyncd.conf > /etc/systemd/timesyncd.conf
sed -i -e "/^NTP=/s/$/ntp.nict.jp/" /etc/systemd/timesyncd.conf > /etc/systemd/timesyncd.conf
sed -i -e "/^FallbackNTP=/s/=.*$/ntp1.jst.mfeed.ad.jp ntp2.jst.mfeed.ad.jp ntp3.jst.mfeed.ad.jp/" /etc/systemd/timesyncd.conf > /etc/systemd/timesyncd.conf
...
# Google Chrome
sudo echo "deb [arch=amd64] http://dl.google.com/linux/chrome/deb/ stable main" | sudo tee -a /etc/apt/sources.list.d/google-chrome.list && wget https://dl.google.com/linux/linux_signing_key.pub && sudo apt-key add linux_signing_key.pub && sudo apt update && sudo apt install -y google-chrome-stable
